...
"""
steady1 = DCSource(amplitude=mean, start=start, stop=stop)
cells[0].inject(steady1)

steady2 = DCSource(amplitude=mean, start=start, stop=stop)
cells[1].inject(steady2)

noise1 = NoisyCurrentSource(mean=mean, stdev=stdev, start=start, stop=stop, dt=10)
cells[0].inject(noise1)

noise2 = NoisyCurrentSource(mean=mean, stdev=stdev, start=start, stop=stop, dt=10)
cells[1].inject(noise2)
"""
...
if '--plot-figure' in sys.argv:
    ...
    plt.xlim(49, 52)
    plt.ylim(-65.1, -64.9)
    plt.annotate('Zoomed In', xy=(51, -65),  weight='bold')
